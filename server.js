...
// STEP 2.1 logic phan loai nguong (THUAN TUY, khong Postgres) de co the unit-test
// toan bo ma tran Case A-J ma khong can DB that (xem storage-policy.js va
// test/storage-policy.test.js). STEP 2.2 them parseConfigNumber() de sua loi
// `parseFloat(...) || default` nuot mat 0/NaN (xem chu thich o loadNumberEnv duoi day).
const { validateThresholds, classifyUsage, formatDiagnostic, parseConfigNumber } = require('./storage-policy');
...
// STEP 2.2 (phat hien boi review doc lap): TRUOC DAY dung pattern
// `parseFloat(process.env.X) || default`, khien X=0 hoac X=abc (go sai) am
// tham roi vao default ma KHONG BAO GIO bao loi - vi 0 va NaN deu falsy trong
// JS. loadNumberEnv() thay the hoan toan pattern do bang parseConfigNumber()
// (storage-policy.js, thuan tuy/co test rieng): CHUA CAU HINH (blank) moi
// duoc dung default; CO CAU HINH nhung khong phai so hop le se FAIL FAST
// (process.exit) thay vi am tham dung default.
function loadNumberEnv(name, defaultValue) {
    const res = parseConfigNumber(process.env[name]);
    if (res.blank) return defaultValue;
    if (!res.valid) {
        console.error(`❌ Biến môi trường ${name} không hợp lệ: ${res.error}. Server từ chối khởi động.`);
        process.exit(1);
    }
    return res.value;
}

const MESSAGE_RETENTION_HOURS = loadNumberEnv('MESSAGE_RETENTION_HOURS', 48);
const DB_STORAGE_LIMIT_MB = loadNumberEnv('DB_STORAGE_LIMIT_MB', null); // blank -> null (tinh nang tat), khong con bi NaN "abc" am tham thanh null ma khong ai biet
const DB_WARNING_RATIO = loadNumberEnv('DB_WARNING_RATIO', 0.80);
const DB_EMERGENCY_RATIO = loadNumberEnv('DB_EMERGENCY_RATIO', 0.90);
const DB_TARGET_RATIO = loadNumberEnv('DB_TARGET_RATIO', 0.75);
...
const DB_HARD_BLOCK_MEDIA_RATIO = loadNumberEnv('DB_HARD_BLOCK_MEDIA_RATIO', 0.95);
const DB_HARD_BLOCK_TEXT_RATIO = loadNumberEnv('DB_HARD_BLOCK_TEXT_RATIO', 0.99);
const EMERGENCY_DELETE_BATCH_SIZE = loadNumberEnv('EMERGENCY_DELETE_BATCH_SIZE', 500);
const CLEANUP_INTERVAL_MS = 10 * 60 * 1000; // 10 phut - nam trong khoang 5-15 phut yeu cau
const MAX_CLEANUP_ITERATIONS_PER_CYCLE = loadNumberEnv('MAX_CLEANUP_ITERATIONS', 50); // STEP 2.1 §14: chan vong lap batch delete chay vo han, co the cau hinh qua MAX_CLEANUP_ITERATIONS

// STEP 2.2: cac gia tri nay PHAI la so duong (0 hoac am deu vo nghia) - kiem
// tra rieng vi loadNumberEnv() chi dam bao "la so hop le", chua dam bao "duong".
// Cac nguong ty le (DB_*_RATIO) da duoc validateThresholds() kiem tra khoang
// (0,1] rieng ben duoi nen khong lap lai o day.
for (const [name, value] of [
    ['MESSAGE_RETENTION_HOURS', MESSAGE_RETENTION_HOURS],
    ['EMERGENCY_DELETE_BATCH_SIZE', EMERGENCY_DELETE_BATCH_SIZE],
    ['MAX_CLEANUP_ITERATIONS', MAX_CLEANUP_ITERATIONS_PER_CYCLE],
]) {
    if (!(value > 0)) {
        console.error(`❌ Biến môi trường ${name}=${value} phải là số dương. Server từ chối khởi động.`);
        process.exit(1);
    }
}
if (DB_STORAGE_LIMIT_MB !== null && !(DB_STORAGE_LIMIT_MB > 0)) {
    console.error(`❌ Biến môi trường DB_STORAGE_LIMIT_MB=${DB_STORAGE_LIMIT_MB} phải là số dương (hoặc để trống để tắt tính năng). Server từ chối khởi động.`);
    process.exit(1);
}
